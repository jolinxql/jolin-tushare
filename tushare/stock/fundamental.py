...
def _get_growth_data(year, quarter, pageNo, dataArr):
    ...
    try:
        request = Request(ct.GROWTH_URL%(ct.P_TYPE['http'], ct.DOMAINS['vsf'],
                                              ct.PAGES['fd'], year,
                                              quarter, pageNo, 200))#ct.PAGE_NUM[1]
        for retry_i in range(10):
            text, html = req_jolin(request)
            if text is not None: break
            print('retry %d'%retry_i, end='')
        res = html.xpath("//table[@class=\"list_table\"]/tr")
        if ct.PY3:
            sarr = [etree.tostring(node).decode('utf-8') for node in res]
        else:
        ...
        print(e)
        print(ct.GROWTH_URL%(ct.P_TYPE['http'], ct.DOMAINS['vsf'],
                             ct.PAGES['fd'], year,
                             quarter, pageNo, ct.PAGE_NUM[1]))
